...
import jwt
import re
from config.config import dbinfo,jwtkey

class auth_model():
    def __init__(self):
        try:
            self.con=mysql.connector.connect(host=dbinfo['hostname'],user=dbinfo['username'],password=dbinfo['password'],database=dbinfo['database'])
